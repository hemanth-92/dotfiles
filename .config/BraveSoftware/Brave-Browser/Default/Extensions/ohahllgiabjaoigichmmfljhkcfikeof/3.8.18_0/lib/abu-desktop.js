...
abu.desktop = (function (abu) {
					...
					try {
						...
						switch(direction) {
							...
							case 'ext_to_app':
								...
								xh.open('POST', `https://${BASE_URL}ext-settings/?key=${key}`, true);
	...
	const DC_KEY = 'abu_desktop_last_check_ts';

	const getDesktopLastCheckTs = () => {
		if (!abu.localStorage.hasItem(DC_KEY)) {
			updateDesktopLastCheckTs();
			return DESKTOP_LAST_CHECK_TS;
		};

		const ts = parseInt(abu.localStorage.getItem(DC_KEY));

		return isNaN(ts) ? 0 : ts;
	}

	const updateDesktopLastCheckTs = () => {
		DESKTOP_LAST_CHECK_TS = Date.now();
		abu.localStorage.setItem(DC_KEY, DESKTOP_LAST_CHECK_TS);
	}

	const getAppKey = (callback) => {
		var xhr = new XMLHttpRequest();
		xhr.open('POST', `https://${BASE_URL}k`, true);
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4) {
				let key = false;
				try {
	...
	const init = () => {
		VERSION = getVersion();
		DESKTOP_LAST_CHECK_TS = getDesktopLastCheckTs();

		if (Date.now() - DESKTOP_LAST_CHECK_TS > PERIOD_CHECK_MS) {
			syncAbuRules();
		}
	}
