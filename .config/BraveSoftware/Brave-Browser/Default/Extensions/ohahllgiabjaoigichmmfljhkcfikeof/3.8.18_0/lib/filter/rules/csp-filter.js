...
(function (abu, api) {
    ...
    api.CspFilter = function (rules) {
        ...
        function findCspRules(url, documentHost, thirdParty, requestType) {
             ...
             * We can't pass `abu.RequestTypes.DOCUMENT` to findRules `function`, because $DOCUMENT modifier has other meaning for the rules.
             * So if we pass `abu.RequestTypes.OTHER` we won't match rules with $SUBDOCUMENT modifier, as we expected
             *
             * For example:
             * rule1 = '||$csp'
             * rule2 = '||$csp,subdocument'
             * rule3 = '||$csp,~subdocument'
             * findCspRules(abu.RequestTypes.SUBDOCUMENT) = [rule1, rule2];
             * findCspRules(abu.RequestTypes.DOCUMENT) = [rule1, rule3];
             */
