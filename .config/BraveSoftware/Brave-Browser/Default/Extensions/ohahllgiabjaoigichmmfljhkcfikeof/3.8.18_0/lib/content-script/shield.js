...
var MainUtils = {
  isShadowRootSupported: function () {
    ...
    return (
      typeof document.documentElement.attachShadow !== 'undefined' &&
...
var Shield = function (params) {
  ...
  const renderSocialLinks = (blockedAds) => {
    return `<div class="social-container">
				<a href="#" data-social="facebook" data-total-ads="${blockedAds}" title="${i18n.getMessage(
      'popup_share_on_fb'
    )}" class="social">
					<i class="fa fa-facebook fa-2x"></i>
				</a>
				<a href="#" data-social="twitter"  data-total-ads="${blockedAds}" title="${i18n.getMessage(
      'popup_share_on_twitter'
    )}" class="social">
												...
												<li class="pure-steps_group-step">
													...
													<fieldset>
														<legend class="pure-steps_group-step_legend">
															...
															<button type="button" class="submit rate-us">${i18n.getMessage(
                                'shield_rate_us'
                              )}</button>
												...
												<li class="pure-steps_group-step">
													<div class="feedback feedback-container">
														...
														<form class="feedback-form">
															<textarea name="msg" placeholder="${i18n.getMessage(
                                'shield_message'
                              )}"></textarea>
															<input type="email" name="email" placeholder="${i18n.getMessage(
                                'shield_email'
                              )}" />
															<button type="submit" class="submit">${i18n.getMessage(
                                'shield_send_feedback'
                              )}</button>
														</form>
													</div>
					...
					<div class="overlay-container-2">
					 ...
					 <div class="form-container log-in-container">
						<div class="form">
						   ...
						   <button class="link" data-url="https://download.adblockultimate.net/AdBlockerInstaller.exe">${i18n.getMessage(
                 'badge_download_now'
               )}</button>
						   <a href="#" data-url="https://adblockultimate.net/windows" class="more link">${i18n.getMessage(
                 'badge_learn_more'
               )}</a>
						   <div class="check">
							  <input type="checkbox" id="show_checkbox" />
							  <label for="show_checkbox"> ${i18n.getMessage(
  ...
  const createIframe = (callback) => {
    ...
    const cssStyle = {
      ...
      'border-radius': '.8em',
      'box-shadow':
        '0 1.7em 5.5em -.94em rgba(0, 0, 0, .3),0 2em 3em .5em rgba(0, 0, 0, .1),0 1.8em 2em -1.5em rgba(0, 0, 0, .2)',
    };
  ...
  const loadIframeContent = (iframe, callback) => {
    ...
    //TODO: local fonts
    const cssLinks = [`${baseUrl}main.css`];
