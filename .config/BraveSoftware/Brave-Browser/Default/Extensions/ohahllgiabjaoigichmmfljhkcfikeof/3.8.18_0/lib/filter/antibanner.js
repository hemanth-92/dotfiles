...
abu.antiBannerService = (function (abu) {
    ...
    /**
     * Period for filters update -- 4 hours
     */
    var UPDATE_FILTERS_PERIOD = 4 * 60 * 60 * 1000;

    /**
     * Period for filters update check -- 15 min
     */
    var UPDATE_FILTERS_CHECK = 15 * 60 * 1000;
    ...
    function createRequestFilter(callback) {
        ...
        var loadFilterRules = function () {
            ...
            abu.localStorage.setItem('initialized_successfully', true);
        };
    ...
    function processSaveFilterRulesToStorageEvents(filterId, events) {
        ...
        abu.rulesStorage.read(filterId, function (loadedRulesText) {
            ...
            for (var i = 0; i < events.length; i++) {
                ...
                switch (eventType) {
                    ...
                    case abu.listeners.REMOVE_RULE:
                        ...
                        break;
                    case abu.listeners.UPDATE_FILTER_RULES:
                        const isCustomFilter = parseInt(filterId) >= abu.subscriptions.CUSTOM_FILTERS_START_ID;
                        const rulesAlreadyText = eventRules.length && typeof eventRules[0] === 'string';

                        if (isCustomFilter && rulesAlreadyText) {
            ...
            if (loadedRulesText[0] && loadedRulesText[0].indexOf('[') === 0) {
                //[Adblock Plus 2.0]
                loadedRulesText.shift();
            }
    ...
    let lastFiltersUpdate = abu.localStorage.getItem('last-filters-update') || 0;
    function scheduleUpdate() {
        ...
        scheduleUpdateTimeoutId = setTimeout(() => {
            ...
            if ((now - lastFiltersUpdate) >= UPDATE_FILTERS_PERIOD) {
                try {
                ...
                abu.localStorage.setItem('last-filters-update', now);
                lastFiltersUpdate = now;
            }

            scheduleUpdate();
        }, UPDATE_FILTERS_CHECK);
    }
