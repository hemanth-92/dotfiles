...
(function (abu) {
    ...
    RequestFilter.prototype = {
         ...
         */
        addRule: function (rule, incrementCounter) {
            if (rule === null || !rule.ruleText) {
                abu.console.error("FilterRule must not be null");
                return;
            }

            if (rule instanceof abu.rules.UrlFilterRule) {
                if (rule.isCspRule()) {
            ...
            } else if (rule instanceof abu.rules.CssFilterRule) {
                this.cssFilter.addRule(rule);
            } else if (rule instanceof abu.rules.ScriptFilterRule || rule instanceof abu.rules.ScriptletRule) {
                this.scriptFilter.addRule(rule);
            }
            if(incrementCounter !== false) this.rulesCount++;
        ...
        getSelectorsForUrl: function (url, genericHide) {
            var domain = abu.utils.url.getHost(url);
         ...
         */
        getScriptsStringForUrl(url) {
            const scriptRules = this.getScriptsForUrl(url);

            const isFirefox = abu.utils.browser.isFirefoxBrowser();
            const isOpera = abu.utils.browser.isOperaBrowser();
        ...
        findWhiteListRule: function (requestUrl, referrer, requestType) {
            ...
            var refHost = abu.utils.url.getHost(referrer);
            var thirdParty = abu.utils.url.isThirdPartyRequest(requestUrl, referrer);
        ...
        findRuleForRequest: function (requestUrl, documentUrl, requestType, documentWhitelistRule) {
            ...
            var documentHost = abu.utils.url.getHost(documentUrl);
            var thirdParty = abu.utils.url.isThirdPartyRequest(requestUrl, documentUrl);
        ...
        _findRuleForRequest: function (requestUrl, documentHost, requestType, thirdParty, documentWhiteListRule) {
            ...
            abu.console.debug("Filtering http request for url: {0}, document: {1}, requestType: {2}", requestUrl, documentHost, requestType);
            ...
            if (urlWhiteListRule &&
                ...
                abu.console.debug("White list rule found {0} for url: {1} document: {2}, requestType: {3}", urlWhiteListRule.ruleText, requestUrl, documentHost, requestType);
                return urlWhiteListRule;
            }

            if (!genericRulesAllowed || !urlRulesAllowed) {
                abu.console.debug("White list rule {0} found for document: {1}", documentWhiteListRule.ruleText, documentHost);
            }
            ...
            if (blockingRule) {
                abu.console.debug("Black list rule {0} found for url: {1}, document: {2}, requestType: {3}", blockingRule.ruleText, requestUrl, documentHost, requestType);
            }
    ...
    abu.RequestFilter = RequestFilter;
