...
(function (abu, api) {
    const stringUtils = abu.utils.strings;
    /**
     * scriptlet mask
     */
    // eslint-disable-next-line no-template-curly-in-string
    const SCRIPTLET_MASK = '${domains}#%#//scriptlet(${args})';

    /**
     * scriptlet exception mask
     */
    // eslint-disable-next-line no-template-curly-in-string
    const SCRIPTLET_EXCEPTION_MASK = '${domains}#@%#//scriptlet(${args})';
    ...
    /**
     * max-length tag for uBlock scripts conversion
     */
    const SCRIPT_MAX_LENGTH = '[max-length="262144"]';
    /**
    ...
    /**
     * Convert string of UBO scriptlet rule to scritlet rule
    ...
    function convertUboScriptletRule(rule) {
        ...
        let template;
        if (mask.indexOf('@') > -1) {
            template = SCRIPTLET_EXCEPTION_MASK;
        } else {
            template = SCRIPTLET_MASK;
        }
    ...
    /**
     * Convert string of ABP scriptlet rule to scritlet rule
     * @param {string} rule UBO scriptlet rule
     */
    ...
    function convertUboScriptTagRule(ruleText) {
        ...
        for (let i = 1; i < match.length; i += 1) {
            ...
            if (isRegExp(attr)) {
                ...
                rules.push(`${domains}${adgScriptTag}[tag-content="${attr}"]${SCRIPT_MAX_LENGTH}`);
            }
        }
    ...
    function convertOptions(rule) {
        ...
        const MP4_OPTION = 'mp4';   //todo
        const CSP_OPTION = 'csp';
        const INLINE_SCRIPT_OPTION = 'inline-script';   //todo
        const INLINE_FONT_OPTION = 'inline-font';   //todo
        const MEDIA_OPTION = 'media';
        const ALL_OPTION = 'all';   //todo
    ...
    /**
     * Convert external scriptlet rule to scriptlet syntax
