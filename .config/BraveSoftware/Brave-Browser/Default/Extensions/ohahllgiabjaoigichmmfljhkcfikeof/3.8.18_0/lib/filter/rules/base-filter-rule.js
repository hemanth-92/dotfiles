...
(function (abu, api) {
    ...
    FilterRule.prototype = {
        ...
        /**
         * Loads $domain option.
         * http://abu.com/en/filterrules.html#hideRulesDomainRestrictions
         * http://abu.com/en/filterrules.html#advanced
         *
         * @param domains List of domains. Examples: "example.com|test.com" or "example.com,test.com"
         */
        loadDomains: function (domains) {

            if (abu.utils.strings.isEmpty(domains)) {
                return;
            }
            ...
            try {
                for (var i = 0; i < parts.length; i++) {
                    ...
                    if (abu.utils.strings.startWith(domain, "~")) {
                        domainName = abu.utils.url.toPunyCode(domain.substring(1).trim());
                        if (!abu.utils.strings.isEmpty(domainName)) {
                        ...
                        domainName = abu.utils.url.toPunyCode(domain.trim());
                        if (!abu.utils.strings.isEmpty(domainName)) {
                            if (permittedDomains === null) {
                                permittedDomains = [];
                            }
                            permittedDomains.push(abu.utils.url.getCroppedDomainName(domainName));
                        }
                    }
                }
            } catch (ex) {
                abu.console.error("Error load domains from {0}, cause {1}", domains, ex);
            }
        ...
        isPermitted: function (domainName) {
            ...
            if (this.restrictedDomain && abu.utils.url.isDomainOrSubDomain(domainName, this.restrictedDomain)) {
                return false;
            }

            if (this.restrictedDomains && abu.utils.url.isDomainOrSubDomainOfAny(domainName, this.restrictedDomains)) {
                return false;
            }

            if (this.hasPermittedDomains()) {
                if (this.permittedDomain && abu.utils.url.isDomainOrSubDomain(domainName, this.permittedDomain)) {
                    return true;
                }

                return abu.utils.url.isDomainOrSubDomainOfAny(domainName, this.permittedDomains);
            }
        ...
        addRestrictedDomains: function (domains) {
            if (domains) {
                ...
                restrictedDomains = abu.utils.collections.removeDuplicates((restrictedDomains || []).concat(domains));
                this.setRestrictedDomains(restrictedDomains);
            }
        },
        ...
        removeRestrictedDomains: function (domains) {
            if (domains) {
                ...
                for (var i = 0; i < domains.length; i++) {
                    abu.utils.collections.remove(restrictedDomains, domains[i]);
                }
                this.setRestrictedDomains(restrictedDomains);
            }
        }
